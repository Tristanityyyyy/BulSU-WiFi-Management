    ...
    <tr className="border-b border-slate-100">
      {Array.from({ length: cols }).map((_, i) => (
        <td key={i} className="px-4 py-3">
          <div className="h-3.5 bg-slate-200 rounded-full animate-pulse" style={{ width: `${60 + (i % 3) * 20}%` }} />
        </td>
      ))}
    </tr>
  );
...
export function SkeletonBlock({ className = "h-48" }) {
  return (
    <div className={`bg-white rounded-2xl shadow-sm border border-slate-200 animate-pulse ${className}`}>
